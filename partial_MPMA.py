...
if __name__ == "__main__":
    ...
    parser.add_argument('maxScore', metavar='t', type=str, help='seed')
    ...
    for epoch in range(100000):
        ...
        for iter in range(max_iter_ITS):
            ...
            local_searches.tabuCol_numba.partialcol_LatinSquare[blockspergrid1, threadsperblock](rng_states,  size_pop,nb_iter_tabu, A_global_mem,  listCols_global_mem, vect_nb_admissible_colors_mem, offsprings_pop_gpu_memory, fitness_offsprings_gpu_memory, tabuTenure_gpu_memory, alpha)
            ...
            if(min(fitness_offsprings_after_tabu) == 0):
                break
            ...
            if(iter < max_iter_ITS - 1):
                ...
                local_searches.tabuCol_numba.partialcol_LatinSquareNotAffectedNode[blockspergrid1, threadsperblock](rng_states,  size_pop,nb_iter_tabu, A_global_mem,  listCols_global_mem, vect_nb_admissible_colors_mem, offsprings_pop_gpu_memory, fitness_offsprings_gpu_memory, tabuTenure_gpu_memory, alpha)




                utils.tools_numba.fileNodeNoColorv2[blockspergrid1, threadsperblock](rng_states, size_pop, offsprings_pop_gpu_memory)
        ...
        utils.tools_numba.computeConflicts[blockspergrid1, threadsperblock]( size_pop, A_global_mem, offsprings_pop_gpu_memory,  fitness_offsprings_gpu_memory)
        fitness_offsprings_after_tabu = fitness_offsprings_gpu_memory.copy_to_host()

        print("nb conflicts max")
        print(np.max(fitness_offsprings_after_tabu))
        ...
        #print("np.min(fitness_offsprings_after_tabu)")
        #print(np.min(fitness_offsprings_after_tabu))
        ...
        if(time() - time_start >budget_time_total or best_score < 1):
            break

        if(best_score + nb_impossible_to_fill == int(args.maxScore)):
            break
        ...
        crossovers.crossovers_numba.compute_nearest_neighbor_crossovers_Latin_Square_partial[blockspergrid1, threadsperblock](
                         rng_states,
                size_pop,
